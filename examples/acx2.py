...
class ACXModel():
  ...
  def __init__(self):
    ...
    self.thermal_broadening = False
    self.broaden_limit = False
    self.velocity_broadening = 0.0
    self.velocity_broadening_units='km/s'
...
#    if DEBUG:
  ...
  def set_broadening(self, thermal_broadening, broaden_limit=False,\
    ...
    if not velocity_broadening_units.lower() in ['km/s']:
      raise pyatomdb.util.UnitsError("Error: velocity broadening units of %s is not in allowed set %s."%\
    ...
    if thermal_broaden_temperature is not None:
      T = pyatomdb.util.convert_temp(thermal_broaden_temperature, teunit, 'K')
      self.thermal_broaden_temperature=T
    else:
      self.thermal_broaden_temperature=None

    for donor in self.DonorList:
      donor.set_broadening(thermal_broadening, broaden_limit=broaden_limit,\
                           velocity_broadening=velocity_broadening, \
                           velocity_broadening_units=velocity_broadening_units,\
...
#    input('DON3')
    ...
    if self.parent is not None:
      thermal_broadening = parent.thermal_broadening
      broaden_limit = parent.broaden_limit
      velocity_broadening=parent.velocity_broadening
      thermal_broaden_temperature=parent.thermal_broaden_temperature
      velocity_broadening_units=parent.velocity_broadening_units


      self.set_broadening(thermal_broadening, \
...
#        if not Z in self.ionfrac.keys():
  ...
  def set_temperature(self, temperature):
    ...
    self.ionfrac_from_temperature = True
    self.temperature = temperature
    self.T =  pyatomdb.util.convert_temp(temperature, 'keV','K')

    for Z in self.spectra.keys():
      for z1 in self.spectra[Z].keys():
        self.spectra[Z][z1].set_temperature(temperature)
  ...
  def set_broadening(self, thermal_broadening, broaden_limit=False,\
    ...
    for Z in self.spectra.keys():
      for z1 in self.spectra[Z].keys():
        self.spectra[Z][z1].set_broadening(thermal_broadening, broaden_limit=broaden_limit,\
                           velocity_broadening=velocity_broadening, \
                           velocity_broadening_units=velocity_broadening_units,\
                           thermal_broaden_temperature=thermal_broaden_temperature,\
                           teunit=teunit)
...
class CXIonSpectrum():
  ...
  def __init__(self, Z, z1, ebins, crosssectiondata,\
               linedata, contdata, cxdata, parent=None):
    """
    initiatlize
...
#        calc_single_spectrum(ebins, nn, l,linedata, contdata)
      ...
      for l in self.spectra[n].keys():
        try:
          for s in self.spectra[n][l].keys():
            self.spectra[n][l][s].set_broadening(thermal_broadening, broaden_limit=broaden_limit,\
                           velocity_broadening=velocity_broadening, \
                           velocity_broadening_units=velocity_broadening_units,\
                           thermal_broaden_temperature=thermal_broaden_temperature,\
                           teunit=teunit)
        except:
          self.spectra[n][l].set_broadening(thermal_broadening, broaden_limit=broaden_limit,\
                           velocity_broadening=velocity_broadening, \
                           velocity_broadening_units=velocity_broadening_units,\
                           thermal_broaden_temperature=thermal_broaden_temperature,\
                           teunit=teunit)
  ...
  def set_temperature(self, T, teunit='K'):
    ...
    T = pyatomdb.util.convert_temp(T, teunit, 'K')
    self.T=T
    ...
    for n in self.spectra.keys():
      for l in self.spectra[n].keys():
        try:
          for s in self.spectra[n][l].keys():
            self.spectra[n][l][s].set_temperature(T, teunit='K')
        except:
          self.spectra[n][l].set_temperature(T, teunit='K')
      ...
      if not l in self.spectra[nn].keys():
        ...
        try:
          self.spectra[nn][l] = CXShellSpectrum(self.Z, self.z1, nn, l, \
                                               ...
                                               self.contdata[self.contdataindexes[nn][l]].data, parent=self)
        except KeyError:
          # Case where there is no data for this Z, z1, nn, l
          self.spectra[nn][l] = DummyCXShellSpectrum(self.Z, self.z1, nn, l, self)
...
class CXIonSpectrum_NLS(CXIonSpectrum):
  ...
  def __init__(self, Z,z1,crosssectiondata, \
    ...
    if self.parent is not None:
      ...
      velocity_broadening_units=parent.velocity_broadening_units
      self.T = parent.T
#      print("Setting self.T = parent.T", self.T, parent.T)
        ...
        if not sig['S2p1'] in self.spectra[sig['n']][sig['l']].keys():
          try:
            self.spectra[sig['n']][sig['l']][sig['S2p1']] = \
               ...
               self.contdata[self.contdataindexes[sig['n']][sig['l']][sig['S2p1']]].data, parent=self)
          except KeyError:
          # Case where there is no data for this Z, z1, nn, l
            self.spectra[sig['n']][sig['l']][sig['S2p1']] = \
               DummyCXShellSpectrum(self.Z, self.z1,sig['n'],sig['l'], s = sig['S2p1'], parent=self)
        ...
        if not l in self.spectra[sig['n']].keys():
          try:
            self.spectra[sig['n']][l] =\
               ...
               self.contdata[self.contdataindexes[sig['n']][l]].data, parent=self)
          except KeyError:
          # Case where there is no data for this Z, z1, nn, l
            self.spectra[sig['n']][l] = \
               DummyCXShellSpectrum(self.Z, self.z1,sig['n'],l, parent=self)
...
to recombination into this shell.
    ...
    if self.parent is not None:
      ...
      thermal_broaden_temperature=parent.thermal_broaden_temperature
      velocity_broadening_units=parent.velocity_broadening_units
#      print("Initializing CXShellSpectrum self.T = ", parent.T)
      self.T = parent.T
...
#      print("self.thermal_broadening: ",self.thermal_broadening,\
#            ", velocity_broadening: ", velocity_broadening,\
#            ", Z:", self.Z,\
#            ", z1:", self.z1,\
#            ", n:", self.n,\
...
#            ", s:", self.s)
      ...
      if ((self.thermal_broadening == False) & \
          (velocity_broadening <=0.0)):
        ...
        ind = self.linedata['Epsilon']>self.broaden_limit
        nonind = ~ind

        # calculate the widths of the strong lines
        llist = self.linedata[ind]
        ...
        if self.thermal_broadening==False:
          ...
          if self.thermal_broaden_temperature is not None:
            T = self.thermal_broaden_temperature
          else:
            T = self.T
...
#        print("Broadeninging %i lines, in %f seconds"%(len(igood), t1-t0))
        ...
        s,z = numpy.histogram(en[nonind], \
                              bins = ebins,\
                              weights = self.linedata['Epsilon'][nonind])
        spec+=s
        self.spectrum = spec
...
class DummyCXShellSpectrum():
  ...
  """
  def __init__(self, Z, z1, n, l, s=False, parent=None):
    """
    Initialization Function
